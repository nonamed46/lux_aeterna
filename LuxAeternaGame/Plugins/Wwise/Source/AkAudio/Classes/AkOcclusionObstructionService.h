...
struct FAkListenerOcclusionObstructionPair
{
	...
	void AsyncTraceFromSource(const FVector& SourcePosition, const FVector& EndPosition, int BoundingBoxPointIndex, ECollisionChannel CollisionChannel, UWorld* World);
	/** Trace a ray from a listener position to a bounding box point asynchronously */
	void AsyncTraceFromListener(const FVector& ListenerPosition, const FVector& EndPosition, int BoundingBoxPointIndex, ECollisionChannel CollisionChannel, UWorld* World);
...
private:
	...
	*/
	...
	TFunction<void(AkGameObjectID, float)> SetOcclusionObstructionFcn;
};